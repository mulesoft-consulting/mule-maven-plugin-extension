...
public abstract class Deployment {
  ...
  @Parameter
  private String exchangeAsset;

  @Parameter
  private String exchangeAssetVersion;
  ...
  public String getExchangeAssetVersion() {
    return exchangeAssetVersion;
  }

  public void setExchangeAssetVersion(String exchangeAssetVersion) {
  ...
  public void setExchangeAsset(String exchangeAsset) {
  ...
  protected void setBasicDeploymentValues(MavenProject project) throws DeploymentException {
    ...
    String exchangeAsset = getProperty("mule.exchangeAsset");
    if (isNotBlank(exchangeAsset)) {
      setExchangeAsset(exchangeAsset);
    }

    // todo verificar que esto se mande si lo de arriba es true
    String exchangeAssetVersion = getProperty("mule.exchangeAssetVersion");
    if (isNotBlank(exchangeAssetVersion)) {
      setExchangeAssetVersion(exchangeAssetVersion);
    }

    if (getArtifact() == null) {
      if (project.getAttachedArtifacts().isEmpty() && !isNotBlank(exchangeAsset)) {
      ...
      } else if (!isNotBlank(exchangeAsset)) {
