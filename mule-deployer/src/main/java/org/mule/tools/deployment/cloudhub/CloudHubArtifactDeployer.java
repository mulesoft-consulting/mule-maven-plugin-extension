...
public class CloudHubArtifactDeployer implements ArtifactDeployer {
  ...
  private Application getApplication(Application originalApplication) {
    log.info("DEPLOYING AN INCREDIBLE APPLICATION");
    Application application = new Application();
    Integer workersAmount;
    String workerType;
    MuleVersion muleVersion = new MuleVersion();

    if (deployment.getExchangeAsset() != null) {
      log.info("Deploying app from Exchange");
      application.setWorkers(getWorkers(DEFAULT_CH_WORKERS, DEFAULT_CH_WORKER_TYPE));
      application.setDomain(deployment.getApplicationName());
      //application.setMuleVersion(muleVersion);
      application.setAutoStart(true);
      Map<String, String> toSave = new HashMap<>();
      toSave.put("source", "EXCHANGE");
      toSave.put("groupId", deployment.getGroupId());
      toSave.put("artifactId", deployment.getExchangeAsset());
      toSave.put("version", deployment.getExchangeAssetVersion());
      toSave.put("organizationId", deployment.getGroupId());
      application.setApplicationSource(toSave);

      Map<String, Object> appInfo = new HashMap<>();
      appInfo.put("domain", application.getDomain());
      appInfo.put("workers", application.getWorkers());
      application.setApplicationInfo(appInfo);
