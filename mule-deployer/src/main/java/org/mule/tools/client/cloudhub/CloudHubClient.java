...
public class CloudHubClient extends AbstractMuleClient {
  ...
  public Application createApplication(Application application, File file) {
    ...
    Entity<Object> entity = getMultiPartEntity(application, file);
  ...
  private Entity<Object> getMultiPartEntity(Application application, File file) {
    FormDataBodyPart appInfoJsonPart = null;
    MultiPart multipart = null;

    if (file != null) {
      FileDataBodyPart filePart = new FileDataBodyPart("file", file);
      appInfoJsonPart = new FormDataBodyPart("appInfoJson", new Gson().toJson(application));
      multipart = new FormDataMultiPart().bodyPart(filePart).bodyPart(appInfoJsonPart);
    } else {
      Gson gson = new Gson().newBuilder().excludeFieldsWithModifiers(Modifier.TRANSIENT).create();
      appInfoJsonPart = new FormDataBodyPart("appInfoJson", gson.toJson(application));
      multipart = new FormDataMultiPart().bodyPart(appInfoJsonPart);
      return Entity.entity(gson.toJson(application), MediaType.APPLICATION_JSON);
    }

    log.info("Entity1: " + appInfoJsonPart.getValue());
    return (file != null) ? Entity.entity(multipart, multipart.getMediaType())
        : Entity.entity(multipart, MediaType.APPLICATION_JSON);
  }
