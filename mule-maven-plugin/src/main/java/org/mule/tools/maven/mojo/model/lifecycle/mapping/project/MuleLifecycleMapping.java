...
public class MuleLifecycleMapping implements LifecycleMapping, ProjectLifecycleMapping {
  ...
  private static final String EXCHANGE_DEPLOY = "exchangeDeploy";
  ...
  public <V> Map<String, V> getLifecyclePhases(LifecycleMappingMavenVersionless mapping) {
    ...
    String isExchangeDeploy = System.getProperty(EXCHANGE_DEPLOY);
    ...
    if (isExchangeDeploy == null || !isExchangeDeploy.equals("true")) {
    ...
    } else if (isExchangeDeploy != null && isExchangeDeploy.equals("true")) {
      phases.put(DEPLOY.id(), mapping.buildGoals("org.mule.tools.maven:mule-maven-plugin:deploy-exchange"));
    } else {
